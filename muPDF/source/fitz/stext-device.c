...
add_block_to_page(fz_context *ctx, fz_stext_page *page)
{
	...
	block->bbox = fz_make_rect(0, 0, 0, 0);
    page->blockcount++;
	return block;
}
...
add_text_block_to_page(fz_context *ctx, fz_stext_page *page)
{
	...
	block->u.t.first_line = block->u.t.last_line = NULL;
	return block;
}
...
add_char_to_line(fz_context *ctx, fz_stext_page *page, fz_stext_line *line, fz_matrix trm, fz_font *font, float size, int c, int g, fz_point *p, fz_point *q, int color, int reender)
{
    ...
    if ((g > 0) && (fz_font_ft_face(ctx, font))) {
		fz_rect rct = fz_bound_glyph(ctx, font, g, trm);
		if (!fz_is_empty_rect(rct)) {
			ch->quad = fz_quad_from_rect(rct);
		}
    } else if (c == ' ') {
    //    a.x = -0.2f;
    //    d.x = 0;
    ...
    //    //ch->bbox.x0 = min4(p->x + a.x, q->x + a.x, p->x + d.x, q->x + d.x); keep x
    //    //ch->bbox.x1 = max4(p->x + a.x, q->x + a.x, p->x + d.x, q->x + d.x);
    //    ch->quad.y0 = min4(p->y + a.y, q->y + a.y, p->y + d.y, q->y + d.y);
    //    ch->quad.y1 = max4(p->y + a.y, q->y + a.y, p->y + d.y, q->y + d.y);
    }
	return ch;
}
...
static void
fz_add_stext_char_imp(fz_context *ctx, fz_stext_device *dev, fz_font *font, int c, int glyph, int oglyph, fz_matrix trm, float adv, int wmode, int force_new_line, int reender)
{
	...
	if (cur_line && glyph < 0)
	{
		...
		add_char_to_line(ctx, page, cur_line, trm, font, size, c, oglyph, &dev->pen, &dev->pen, dev->color, reender);
		dev->lastchar = c;
		return;
	}
	...
	else
	{
		...
		if (fabsf(base_offset) < size * 0.8f)
		{
			...
			if (dev->curdir >= 0)
			{
				...
				else //CCS: if (spacing >= size * SPACE_DIST && spacing < size * SPACE_MAX_DIST)
                    if ((q.y - dev->pen.y) < (size / 2))
				{
	...
	if (add_space)
		add_char_to_line(ctx, page, cur_line, trm, font, size, ' ', -1, &dev->pen, &p, dev->color, reender);

	add_char_to_line(ctx, page, cur_line, trm, font, size, c, oglyph, &p, &q, dev->color, reender);
	dev->lastchar = c;
	dev->pen = q;
...
static void
fz_add_stext_char(fz_context *ctx, fz_stext_device *dev, fz_font *font, int c, int glyph, fz_matrix trm, float adv, int wmode, int force_new_line, int reender)
{
	...
	if (!(dev->flags & FZ_STEXT_PRESERVE_LIGATURES))
	{
		...
		{
		case 0xFB00: /* ff */
            fz_add_stext_char_imp(ctx, dev, font, 'f', glyph, glyph, trm, adv, wmode, force_new_line, reender);
            fz_add_stext_char_imp(ctx, dev, font, 'f', -1, glyph, trm, 0, wmode, 0, reender);
			return;
		case 0xFB01: /* fi */
            fz_add_stext_char_imp(ctx, dev, font, 'f', glyph, glyph, trm, adv, wmode, force_new_line, reender);
            fz_add_stext_char_imp(ctx, dev, font, 'i', -1, glyph, trm, 0, wmode, 0, reender);
			return;
		case 0xFB02: /* fl */
            fz_add_stext_char_imp(ctx, dev, font, 'f', glyph, glyph, trm, adv, wmode, force_new_line, reender);
            fz_add_stext_char_imp(ctx, dev, font, 'l', -1, glyph, trm, 0, wmode, 0, reender);
			return;
		case 0xFB03: /* ffi */
            fz_add_stext_char_imp(ctx, dev, font, 'f', glyph, glyph, trm, adv, wmode, force_new_line, reender);
            fz_add_stext_char_imp(ctx, dev, font, 'f', -1, glyph, trm, 0, wmode, 0, reender);
		...
		case 0xFB05: /* long st */
		case 0xFB06: /* st */
            fz_add_stext_char_imp(ctx, dev, font, 's', glyph, glyph, trm, adv, wmode, force_new_line, reender);
            fz_add_stext_char_imp(ctx, dev, font, 't', -1, glyph, trm, 0, wmode, 0, reender);
			return;
		}
	}
    ...
    fz_add_stext_char_imp(ctx, dev, font, c, glyph, glyph, trm, adv, wmode, force_new_line, reender);
}

static void
fz_stext_extract(fz_context *ctx, fz_stext_device *dev, fz_text_span *span, fz_matrix ctm, int reender)
{
	...
	fz_colorspace *colorspace, const float *color, float alpha, fz_color_params color_params)
{
	...
	tdev->new_obj = 1;
	for (span = text->head; span; span = span->next)
		fz_stext_extract(ctx, tdev, span, ctm, 1);
	fz_drop_text(ctx, tdev->lasttext);
	tdev->lasttext = fz_keep_text(ctx, text);
}
...
fz_stext_clip_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm, fz_rect scissor)
{
	...
	tdev->new_obj = 1;
	for (span = text->head; span; span = span->next)
		fz_stext_extract(ctx, tdev, span, ctm, 7);
	fz_drop_text(ctx, tdev->lasttext);
	tdev->lasttext = fz_keep_text(ctx, text);
}
...
fz_stext_clip_stroke_text(fz_context *ctx, fz_device *dev, const fz_text *text, const fz_stroke_state *stroke, fz_matrix ctm, fz_rect scissor)
{
	...
	tdev->new_obj = 1;
	for (span = text->head; span; span = span->next)
		fz_stext_extract(ctx, tdev, span, ctm, 5);
	fz_drop_text(ctx, tdev->lasttext);
	tdev->lasttext = fz_keep_text(ctx, text);
}
...
fz_stext_ignore_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm)
{
	...
	tdev->new_obj = 1;
	for (span = text->head; span; span = span->next)
		fz_stext_extract(ctx, tdev, span, ctm, 3);
	fz_drop_text(ctx, tdev->lasttext);
	tdev->lasttext = fz_keep_text(ctx, text);
}
