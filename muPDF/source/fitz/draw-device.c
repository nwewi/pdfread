...
{
#ifdef _DEBUG
	printf(" draw_set_default_colorspace");
#endif
	fz_draw_device *dev = (fz_draw_device*)devp;
...
//[CCS
static void fz_draw_fill_path_none(fz_context *ctx, fz_device *devp, const fz_path *path, int even_odd, fz_matrix in_ctm,
								   fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_fill_path-none");
#endif
	return;
}
static void fz_draw_stroke_path_none(fz_context *ctx, fz_device *devp, const fz_path *path, const fz_stroke_state *stroke, fz_matrix in_ctm,
									 fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_stroke_path-none");
#endif
	//fz_draw_device *dev = (fz_draw_device *)devp;
	//fz_draw_state *state = &dev->stack[dev->top];
	//state = push_stack(ctx, dev, "stroke path");
}
static void fz_draw_fill_text_none(fz_context *ctx, fz_device *devp, const fz_text *text, fz_matrix in_ctm,
								   fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_fill_text-none");
#endif
	return;
}
static void fz_draw_stroke_text_none(fz_context *ctx, fz_device *devp, const fz_text *text, const fz_stroke_state *stroke,
									 fz_matrix in_ctm, fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_stroke_text-none");
#endif
	return;
}
static void fz_draw_clip_path_none(fz_context *ctx, fz_device *devp, const fz_path *path, int even_odd, fz_matrix in_ctm, fz_rect scissor)
{
#ifdef _DEBUG
	printf(" draw_clip_path-none");
#endif
	fz_draw_device *dev = (fz_draw_device *)devp;
    fz_draw_state *state = &dev->stack[dev->top];
    state = push_stack(ctx, dev,"clip path");
...
static void fz_draw_clip_stroke_path_none(fz_context *ctx, fz_device *devp, const fz_path *path, const fz_stroke_state *stroke, fz_matrix in_ctm, fz_rect scissor)
{
#ifdef _DEBUG
	printf(" draw_clip_stroke_path-none");
#endif
	fz_draw_device *dev = (fz_draw_device *)devp;
	fz_draw_state *state = push_stack(ctx, dev, "clip stroke path");
	state[1].scissor = fz_empty_irect;
	state[1].mask = NULL;
}
static void fz_draw_clip_text_none(fz_context *ctx, fz_device *devp, const fz_text *text, fz_matrix in_ctm, fz_rect scissor)
{
	fz_irect bbox;
	fz_colorspace *model;
	fz_draw_device *dev = (fz_draw_device*)devp;
...
#ifdef _DEBUG
	printf(" draw_clip_text-none"); 
#endif
    fz_draw_state *state = &dev->stack[dev->top];
    state = push_stack(ctx, dev,"clip text none");
	model = state->dest->colorspace;
...
#ifdef DUMP_GROUP_BLENDS
	dump_spaces(dev->top - 1, "Clip (text) none begin\n");
#endif
}
static void fz_draw_clip_stroke_text_none(fz_context *ctx, fz_device *devp, const fz_text *text, const fz_stroke_state *stroke, fz_matrix in_ctm, fz_rect scissor)
{
#ifdef _DEBUG
	printf(" draw_clip_stroke_text-none");
#endif
	fz_draw_device *dev = (fz_draw_device *)devp;
    fz_draw_state *state = &dev->stack[dev->top];
    state = push_stack(ctx, dev,"stroke text");
	state[1].scissor = fz_empty_irect;
	state[1].mask = NULL;
}
static void fz_draw_ignore_text_none(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm)
{
#ifdef _DEBUG
	printf(" draw_ignore_text-none");
#endif
}
static void fz_draw_fill_image_mask_none(fz_context *ctx, fz_device *devp, fz_image *image, fz_matrix in_ctm,
										 fz_colorspace *colorspace_in, const float *color, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_fill_image_mask-none");
#endif
}
static void fz_draw_clip_image_mask_none(fz_context *ctx, fz_device *devp, fz_image *image, fz_matrix in_ctm, fz_rect scissor)
{
#ifdef _DEBUG
	printf("draw_clip_image_mask-none");
#endif
	fz_draw_device *dev = (fz_draw_device *)devp;
    fz_draw_state *state = &dev->stack[dev->top];
    state = push_stack(ctx, dev,"clip image mask");
	state[1].scissor = fz_empty_irect;
	state[1].mask = NULL;
}

static void fz_draw_fill_image_none(fz_context *ctx, fz_device *devp, fz_image *image, fz_matrix in_ctm, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_fill_image-none");
#endif
}
static void fz_draw_fill_shade_none(fz_context *ctx, fz_device *devp, fz_shade *shade, fz_matrix in_ctm, float alpha, fz_color_params color_params)
{
#ifdef _DEBUG
	printf(" draw_fill_shade-none");
#endif
}
fz_device * fz_new_imagesonly_device(fz_context *ctx, const fz_matrix transform, fz_pixmap *dest)
{
    fz_draw_device *dev = (fz_draw_device*)new_draw_device(ctx, transform, dest, NULL, NULL, NULL);

    dev->super.fill_path = fz_draw_fill_path_none;
    dev->super.stroke_path = fz_draw_stroke_path_none;
    //dev->super.clip_path = fz_draw_clip_path;
    //dev->super.clip_stroke_path = fz_draw_clip_stroke_path;

    dev->super.fill_text = fz_draw_fill_text_none;
    dev->super.stroke_text = fz_draw_stroke_text_none;
    dev->super.clip_text = fz_draw_clip_text_none;
    dev->super.clip_stroke_text = fz_draw_clip_stroke_text_none;
    dev->super.ignore_text = fz_draw_ignore_text_none;

    dev->super.fill_image_mask = fz_draw_fill_image_mask_none;
    dev->super.clip_image_mask = fz_draw_clip_image_mask_none;
    //dev->super.fill_image = fz_draw_fill_image;
    dev->super.fill_shade = fz_draw_fill_shade_none;
...
fz_device * fz_new_pathsonly_device(fz_context *ctx, const fz_matrix transform, fz_pixmap *dest)
{
    fz_draw_device *dev = (fz_draw_device*)new_draw_device(ctx, transform, dest, NULL, NULL, NULL);
    ...
    dev->super.fill_image_mask = fz_draw_fill_image_mask_none;
    dev->super.clip_image_mask = fz_draw_clip_image_mask_none;
    dev->super.fill_image = fz_draw_fill_image_none;
    dev->super.fill_shade = fz_draw_fill_shade_none;
...
fz_device * fz_new_imagesandpathsonly_device(fz_context *ctx, const fz_matrix transform, fz_pixmap *dest)
{
    fz_draw_device *dev = (fz_draw_device*)new_draw_device(ctx, transform, dest, NULL, NULL, NULL);
...
fz_device * fz_new_textsonly_device(fz_context *ctx, const fz_matrix transform, fz_pixmap *dest)
{
    fz_draw_device *dev = (fz_draw_device*)new_draw_device(ctx, transform, dest, NULL, NULL, NULL);

    dev->super.fill_path = fz_draw_fill_path_none;
    dev->super.stroke_path = fz_draw_stroke_path_none;
    dev->super.clip_path = fz_draw_clip_path_none;
    dev->super.clip_stroke_path = fz_draw_clip_stroke_path_none;
