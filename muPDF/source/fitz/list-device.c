...
int
imageIsValid(fz_image* image)
{
    if (image == NULL) return 0;
    // old later additional checks
    //if (image->buffer == NULL) return 0;
    //if (image->buffer->buffer == NULL) return 0;
    //if (image->buffer->buffer->data == NULL) return 0;

    return 1;
}

int
isBKGImage(fz_image* image, fz_rect* pageBounds)
{
    if (image == NULL || pageBounds == NULL) {
        return 1; //image should be ignored anyway
    }
    //should be less than 1% difference 
    if (((double)image->w) / abs(pageBounds->x1 - pageBounds->x0) >= 0.99 && ((double)image->h) / abs(pageBounds->y1 - pageBounds->y0) >= 0.99) {
        return 1;
    } else {
        return 0;
    }
...
visible:
		...
		fz_try(ctx)
		{
			...
			case FZ_CMD_FILL_IMAGE:
				fz_unpack_color_params(&color_params, n.flags);
                if ((((dev->hints & FZ_CCS_SKIP_INVALID) == 0) || imageIsValid(*(fz_image **)node)) && (((dev->hints & FZ_CCS_SKIP_BKG_IMAGE) == 0) || !isBKGImage(*(fz_image **)node, &trans_rect))) {
				    fz_fill_image(ctx, dev, *(fz_image **)node, trans_ctm, alpha, color_params);
                } else
                    fz_warn(ctx, "skiped invalid image or background image");
				break;
			case FZ_CMD_FILL_IMAGE_MASK:
				fz_unpack_color_params(&color_params, n.flags);
                if (((dev->hints & FZ_CCS_SKIP_INVALID) == 0) || imageIsValid(*(fz_image **)node)) {
				    fz_fill_image_mask(ctx, dev, *(fz_image **)node, trans_ctm, colorspace, color, alpha, color_params);
                } else
                    fz_warn(ctx, "skiped invalid image");
				break;
			case FZ_CMD_CLIP_IMAGE_MASK:
                if (((dev->hints & FZ_CCS_SKIP_INVALID) == 0) || imageIsValid(*(fz_image **)node))
